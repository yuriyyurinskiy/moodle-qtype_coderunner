...
Версия: 3.6.0 Январь 2019

Авторы: Richard Lobb, University of Canterbury, New Zealand.
        Tim Hunt, The Open University, UK

ПРИМЕЧАНИЕ: Несколько примеров тестов, содержащих примеры вопросов 
CodeRunner доступны на [coderunner.org.nz](http://coderunner.org.nz). Там также есть
[форум](http://coderunner.org.nz/mod/forum/view.php?id=51), где вы сможете
публиковать вопросы CodeRunner, такие как такие как запросы о помощи, 
если что-то пойдет не так, или ищете идеи о том, как написать какой-то необычный тип вопроса

*Прочитать на других языках: [English](Readme.md), [Русский](Readme.ru-RU.md)*
...
<!--ts-->
   * [CODE RUNNER](#code-runner)
      ...
      * [Архитектура CodeRunner](#the-architecture-of-coderunner)
      * [Типы вопроса](#question-types)
         * [Пример типа вопроса](#an-example-question-type)
         * [Встроенные типы вопросов](#built-in-question-types)
         * [Some more-specialised question types](#some-more-specialised-question-types)
      * [Шаблоны](#templates)
         * [Шаблон для каждого контрольного примера](#per-test-templates)
         * [Шаблон комбинатор](#combinator-templates)
...
CodeRunner это тип вопроса СДО Moodle, который позволяет учителям запускать программу 
для оценки ответа ученика. На сегодняшний день CodeRunner больше всего используют 
на курсах программирования, где студентов просят написать программный код 
для некоторого задания, а затем этот код оценивается путем его выполнения 
в серии контрольных примеров. Вопросы CodeRunner также использовались в других областях информатики 
и техники для оценки вопросов, в которых возможно множество различных  правильных ответов, 
и программа должна использоваться для оценки правильности.

Независимо от поведения, выбранного для теста, вопросы CodeRunner всегда выполняются 
в адаптивном режиме, в котором студенты могут нажать кнопку Проверить, чтобы узнать: 
проходит ли их код контрольные примеры, определенные в вопросе. При неудаче, студенты 
могут отправить новый вариант, обычно с небольшим штрафом. В режиме по умолчанию 'Все или ничего', 
все контрольные примеры должны завершиться успешно, и только в этом случае за попытку начисляется 
некоторый балл. В итоге оценка за тест, состоящий из вопросов CodeRunner, зависит в первую очередь 
от вопросов, которые студент смог решить правильно, и во вторую очередь от количества попыток, 
которые студент потратил на каждый вопрос. Тем не менее, также можно настроить вопросы CodeRunner 
таким образом, чтобы оценка зависила только от количества контрольных примеров, которые завершились 
успешно.

CodeRunner используется в Университете Кентербери более семи лет, выполнив миллионы вопросов 
из студенческих тестов на языках Python, C, JavaScript, PHP, Octave и Matlab. 
Лабораторные работы, практические задания и контрольные срезы на вводном первом курсе дисциплины 
программирование Python (COSC121), насчитывающим около 650 студентов в первом семестре и 350 во втором, 
оцениваются с помощью вопросов CodeRunner. Выпускные экзамены для COSC121 также проводились 
с использованием Moodle/CodeRunner с ноября 2014 года. Другие курсы в университете Кентербери 
использующие в своей программе CodeRunner:
...
CodeRunner также используется на более чем 600 других сайтах по всему миру.

CodeRunner в настоящее время поддерживает Python2 (считается устаревшим), Python3, C, C++, Java, 
PHP, JavaScript (NodeJS), Octave и Matlab.
Архитектура позволяет легко расширяться на другие языки.

CodeRunner можно безопасно использовать на основном сервере СДО Moodle организации, 
при условии, что программное обеспечение песочницы, в котором выполняется код ("Jobe"), 
установлено на отдельной машине с адекватной безопасностью и брандмауэром.  Однако, 
если тесты на основе CodeRunner должны использоваться для контрольных срезов и выпускных экзаменов, 
рекомендуется использовать отдельный сервер СДО Moodle, как по причинам загрузки, так и для того, 
чтобы различные средства связи Moodle, такие как чат и обмен сообщениями, можно было отключить, 
не влияя на другие классы.

Тип вопроса CodeRunner может быть установлен в любой современной системе Moodle 
(версии 3.0 или более поздней), на Linux, Windows и Mac. По соображениям безопасности 
отправленные задания выполняются на отдельном компьютере под названием "Jobe server" или 
"Jobe sandbox machine". CodeRunner изначально настроен на использование небольшого внешнего Jobe server 
в Университете Кентербери, и его можно использовать для начального тестирования. 
Однако это не подходит для использования в 'продакшене', для этого организации необходимо 
установить собственный Jobe server.
Инструкции по установке Jobe server приведены в документации к Jobe server.
После установки Jobe server, используйте интерфейс администратора Moodle для плагина CodeRunner, 
чтобы указать имя хоста задания и, возможно, номер порта.

Один 4-ядерный сервер СДО Moodle может обрабатывать в среднем около 60 вопросов теста в минуту, 
сохраняя при этом время ответа менее 3-4 секунд, предполагая, что сам ответ студента выполняется 
за долю секунды. Мы провели испытания CodeRunner с почти 300 студентами и испытали только легкие и 
умеренные факторы нагрузки на 8-ядерном сервере Moodle. Jobe server, 
который выполняет задания студента (см. ниже), еще более легко загружается во время такого испытания.

Некоторые видеоролики, знакомящие с CodeRunner и объясняющие авторство вопросов, 
доступны на [этом youtube канале](https://coderunner.org.nz/mod/url/view.php?id=472).
...
## <a name="the-architecture-of-coderunner"></a> Архитектура CodeRunner

Несмотря на то что соз
...
## <a name="question-types"></a> Типы вопроса

CodeRunner поддерживает широкий спектр типов вопросов и может быть легко расширен 
для поддержки других. Тип вопроса CodeRunner определяется *прототипа вопроса*, 
который определяет параметры времени выполнения, такие как язык выполнения и песочница, 
а также шаблон, который определяет, как тестовая программа строится из контрольного примера 
вопроса и ответа студента. Прототип также определяет, оценивается ли правильность 
ответа студента с помощью *EqualityGrader*, *NearEqualityGrader* или *RegexGrader*. 
EqualityGrader ожидает, что результат выполнения теста будет точно совпадать 
с результатом контрольного примера. NearEqualityGrader работает аналогично, 
но нечувствителен к регистру и допускает изменения количества пробелов 
(например, отсутствующие или дополнительные пустые строки или несколько пробелов, 
где ожидался только один).
RegexGrader ожидает соответствия регулярному выражению. EqualityGrader рекомендуется 
для любого нормального использования, поскольку он побуждает студентов получать 
свои результаты точно правильно; они должны быть в состоянии повторно представить 
почти правильные ответы для небольшого штрафа, что, как правило, лучше, чем пытаться 
присуждать метки частей на основе совпадений регулярных выражений.

Контрольные примеры определяются автором вопроса для проверки ответа студента.
Каждый контрольный пример определяет фрагмент тестового кода, стандартный ввод, 
используемый при запуске тестовой программы, и ожидаемый вывод из этого запуска. 
Автор также может добавить дополнительные файлы в среде выполнения.

Тестовая программа состоит из информации контрольного примера 
и ответа студента с использованием шаблона прототипа вопроса. Шаблон может быть 
либо *шаблон для каждого контрольного примера*, который определяет другую программу 
для каждого контрольного примера, либо шаблоном *шаблон-комбинатор*, который имеет 
возможность определить программу, объединяющую несколько контрольных примеров 
в один запуск. Шаблоны описаны в разделе *Шаблоны*.

### <a name="an-example-question-type"></a> Пример типа вопроса

Тип вопроса C-function ожидает, что студенты представят функцию C, а также возможно 
дополнительные функции для некоторого задания. В качестве тривиального примера 
можно задать вопрос: "Напишите функцию C с сигнатурой `int sqr(int n)`, 
которая возвращает квадрат ее параметра *n*". Затем автор предоставит 
некоторые контрольные примеры вида

    printf("%d\n", sqr(-11));

и даст ожидаемый результат от этого контрольного примера.

Шаблон для каждого контрольного примера для данного типа вопроса обернет решение и 
код контрольного примера в одну программу, такую как:

    #include <stdio.h>

    // --- Ответ студента вставляется здесь ----
...
который будет скомпилирован и запущен для каждого контрольного примера. 
Затем результат выполнения будет сравниваться с указанным ожидаемым результатом (121), 
и контрольный пример будет отмечен соответствующим образом правильно или неправильно.

В этом примере предполагается использование шаблона для каждого контрольного примера, 
а не более сложного шаблона - шаблон-комбинатор, который фактически используется 
встроенным типом вопроса функции C. Дополнительные сведения см. В разделе *Шаблоны*.

### <a name="built-in-question-types"></a> Встроенные типы вопросов

Файл `<moodlehome>/question/type/coderunner/db/builtin_PROTOTYPES.xml` представляет собой 
файл формата экспорта moodle-xml, содержащий определения всех встроенных типов вопросов. 
Во время установки и в конце любого обновления версии прототипы вопросов из этого файла 
загружаются в категорию "CR_PROTOTYPES" в контексте системы. Системный администратор 
может редактировать эти прототипы, но это не рекомендуется, так как все изменения 
будут потеряны при каждом обновлении. Вместо этого следует создать категорию `LOCAL_PROTOTYPES` 
(или другое такое имя по вашему выбору) и сохранять в нее копии любого прототипа вопросов, 
который нуждается в редактировании, с соответствующим изменением имени типа вопроса. 
В этой категории также могут быть созданы новые типы прототипов вопросов. 
Редактирование прототипов рассматривается ниже в этом документе.

Встроенные типы вопросов включают следующие:

 1. **c\_function**. Это типа вопрос обсуждается в приведенном выше примере, 
 за исключением того, что он использует шаблон-комбинатор. Ответ студента 
 должен содержать только функцию (плюс возможные функции поддержки), 
 а каждый контрольный пример (как правило) имет вид

        printf(format_string, func(arg1, arg2, ..))

 Шаблон для этого типа вопросов состоит из некоторые стандартные зависимостей, 
 ответа студента и основной функции, которая выполняет контрольные примеры 
 один за другим. Однако если в каком-либо из контрольных примеров определен 
 стандартный ввод данных, шаблон расширяется и выполняется отдельно 
 для каждого контрольного примера.

 Способ выполнения программы C (или любой другой) не является частью определения 
 типа вопроса: он определяется конкретной песочницей, в которую передается выполнение. 
 Jobe sandbox использует компилятор `gcc` с языком, установленным для принятия C99 и 
 с параметрами *-Wall* и *-Werror*, установленными в командной строке, чтобы выдавать 
 все предупреждения и отклонять код, если есть какие-либо предупреждения.

 1. **cpp\_function**. Это версия C++ предыдущего типа вопроса. 
 Студент поставляет только функцию (плюс возможные функции поддержки), 
 а каждый контрольный пример (как правило) имет вид
 ...
 ответа студента и основной функции, которая выполняет контрольные примеры по одиному.

 1. **c\_program** и **cpp\_program**. Эти два очень простых типа вопросов, 
 требующих от студента предоставления полной рабочей программы. Для каждого 
 контрольного примера автор обычно задает стандартный ввод `stdin` и ожидаемый 
 стандартный вывод `stdout`. Программа компилируется и запускается как есть, 
 в режиме оценивания по умолчанию 'Все или ничего', все контрольные примеры 
...
### <a name="some-more-specialised-question-types"></a> Некоторые специальные типы вопросов
...
Шаблоны являются ключом к пониманию того, как проверяется ответ. Каждый вопрос 
имеет шаблон, импортированный из типа вопроса или явно настроенный, 
который определяет, как исполняемая программа создается из ответа студента, 
кода контрольного примера и другого пользовательского кода в самом шаблоне.

Шаблон вопроса может быть либо *шаблон для каждого контрольного примера*, 
либо *шаблон-комбинатор*. Первый из них проще; он применяется один раз 
для каждого теста в вопросе, чтобы получить исполняемую программу, 
которая отправляется в песочницу. Каждое такое выполнение определяет 
одну строку таблицы результатов. Второй - шаблон-комбинатор, как следует 
из названия, может объединять несколько контрольных примеров в одно выполнение 
при условии отсутствия  стандартного ввода для любого из контрольных примеров. 
Сначала мы обсудим более простой - шаблон для каждого контрольных примеров.

### <a name="per-test-templates"></a> Шаблон для каждого контрольного примера

*Шаблон для каждого контрольного примера* - это, по сути, программа с  "заполнителями", 
в которые вставляются ответ студента и код контрольного примера.
...
Когда Twig обрабатывает шаблон, он заменяет любые вхождения строк формы `{{ TWIG\_VARIABLE }}` 
значением данного TWIG_VARIABLE (например, STUDENT\_ANSWER). 
Например, тип вопроса *c\_function*, который просит студентов написать функцию C, 
может иметь следующий шаблон (если он использовал шаблон для каждого контрольного примера):
...
Обычный контрольный пример (т.е. `TEST.testcode`) для вопроса, 
предлагающего студентам написать функцию, которая возвращает 
квадрат ее параметра, может быть:

        printf("%d\n", sqr(-9))

с ожидаемым стандартным выводом 81. Результатом подстановки 
ответа студента и кода контрольного примера в шаблон будет программа, 
которая может выглядеть следующим образом (в зависимости от ответа студента, конечно):
...
При создании вопроса Вы можете проверить шаблон для выбранного типа вопроса, 
временно установив флажок 'Customise'. 
Кроме того, если вы установите флажок *Template debugging* вы увидите на веб-странице 
вывода каждую из полных программ, которые запускаются во время отправки ответа.

### <a name="combinator-templates"></a> Шаблон-комбинатор
