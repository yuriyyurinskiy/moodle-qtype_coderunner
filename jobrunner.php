...
class qtype_coderunner_jobrunner {
    ...
    public function run_tests($question, $code, $testcases) {
        global $CFG;
        ...
        $sandboxparams = $question->get_sandbox_params();
        ...
        $outcome = $this->run_with_combinator($question, $code, $testcases, $files,
                $sandboxparams, $templateparams);
        ...
        if ($outcome == null) {
            $outcome = $this->run_tests_singly($question, $code, $testcases, $files,
                    $sandboxparams, $templateparams);
        }
    ...
    private function run_with_combinator($question, $code, $testcases, $files,
            $sandboxparams, $templateparams) {
        $combinator = $question->get_combinator();
        $iscombinatorgrader = $this->grader->name() === 'combinatortemplategrader';
        $usecombinator = $iscombinatorgrader
            || (!empty($combinator) && $this->has_no_stdins($testcases) &&
        ...
        $testprog = $this->twig->render($combinator, $templateparams);

        $this->allruns[] = $testprog;
        $run = $this->sandbox->execute($testprog, $question->get_language(),
        ...
        if ($run->error !== qtype_coderunner_sandbox::OK) {
            ...
            $outputs = preg_split($question->get_test_splitter_re(), $run->output);
    ...
    private function run_tests_singly($question, $code, $testcases, $files,
            $sandboxparams, $templateparams) {
        $maxmark = $this->maximum_possible_mark($testcases);
        $outcome = new qtype_coderunner_testing_outcome($maxmark);
        $template = $question->get_per_test_template();
        foreach ($testcases as $testcase) {
            ...
            $this->allruns[] = $testprog;
            $run = $this->sandbox->execute($testprog, $question->language,
